...
class ChatCubit extends Cubit<ChatState> {
  ...
  Future<void> initialize(ChatRouteArgs routeArgs) async {
    ...
    final conversation = conversationResult.fold<ConversationEntity?>(
      (failure) {
        emit(
          state.copyWith(
  ...
  Future<void> sendMessage(String text) async {
    ...
    if (conversation == null || trimmed.isEmpty) {
      return;
    }
...
class ChatCubit extends Cubit<ChatState> {
  ...
  Future<void> initialize(ChatRouteArgs routeArgs) async {
    ...
    final connectionResult = await connectToChatUseCase();
    final isConnected = connectionResult.fold((_) => false, (_) => true);
